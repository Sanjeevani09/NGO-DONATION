...
<body>
        ...
        <nav class="navbar navbar-expand-lg my-navbar">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item">
                        <a href="./donordonations.php" class="nav-link active">My Donations</a>
                    </li>
                    <li class="nav-item">
    ...
    <section>
        <div class="container py-4 my-4 bg-light" style="box-shadow: 2px 2px 2px 2px #777;">
            <?php
            $connection = mysqli_connect("localhost", "root", "", "ngo");
            ...
            $sql = "SELECT * FROM donation_form JOIN users ON donation_form.email=users.email WHERE users.email='$email'";
            $result2 = mysqli_query($connection, $sql);
            if (mysqli_num_rows($result2) > 0) { ?>
                <h2 style="text-align: center; font-weight: bold;">My Donations :
                    <hr class="internship-green-hr">
                </h2>
                <table class="table table-hover">
                    <thead class="thead-light">
                        <?php echo "<tr><th>Donation Date</th><th>No. of packets</th><th>Amount</th><th></th><th></th></tr>"; ?> </thread>

                    <?php while ($row2 = mysqli_fetch_assoc($result2)) {
                        $date = date("d-m-Y", strtotime($row2['date']));
                        echo "<tr><td>" . $date . "</td><td>" . $row2["packets"] . "</td><td>" . $row2["amount"] . "</td><td><a href='certificate.php'><button class='btn btn-outline-success mt-2 '> Download Certificate </button></a></td></tr>";
                    }
                    echo "</table>";
                } else {
                    echo "<tr colspan=3></tr>No Donation History</tr>";
                }
