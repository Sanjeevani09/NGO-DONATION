...
<body>
  ...
  <section>
    ...
    if (isset($_GET['id'])) {
      $eid = $_GET['id'];
      $q = "SELECT * FROM featureevent where id = '$eid'";
...
if (isset($_GET['id'])) {
  ...
  if (isset($_POST['submit'])) {
    ...
    if ($tempname != "") {
      ...
      $stmt = mysqli_query($conn, "SELECT image FROM featureevent WHERE id='$eid'");
      ...
      $q = "UPDATE featureevent SET image='$final_image' WHERE id='$eid'";
      mysqli_query($conn, $q);
      ...
      $delete = "UPDATE featureevent SET name= '$name', date= '$date', description='$description', details='$details',  WHERE id= $eid";
