...
<body>
    ...
    <nav class="navbar navbar-expand-lg my-navbar">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav ml-auto">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="./donations.php">View Donations</a>
          </li>
          <li class="nav-item">
  ...
  <section>
    <div class="container py-4 my-4 bg-light" style="box-shadow: 2px 2px 2px 2px #777;">
      <h2 style="text-align : center; font-weight: bold;">Donations List :
      ...
      <form action="donations.php" method="post">
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="form-group">
              <button type="submit" name="filter_button" class="btn btn-primary">Search</button>
            </div>
          </div>
        </div>
      </form>
      ...
      if (isset($_POST['search'])) {
        ...
        $sql = "SELECT * FROM donation_form WHERE concat(name,phone,email,packets,date) LIKE '%$search%'";
      ...
      ?>
          ...
          <table class="table table-hover">
            <thead class="thead-light">
              <?php echo "<tr><th>Donor Name</th><th>Phone</th><th>Email</th><th>No. of packets</th><th>Amount</th><th>Donation Date</th></tr>"; ?></thread>
          <?php while ($row = mysqli_fetch_assoc($result)) {
            $date = date("d-m-Y", strtotime($row['date']));
            echo "<tr><td>" . $row["name"] . "</td><td>" . $row["phone"] . "</td><td>" . $row["email"] . "</td><td>" . $row["packets"] . "</td><td>" . $row["amount"] . "</td><td>" . $date . "</td></tr>";
          }
          echo "</table>";
        } else {
